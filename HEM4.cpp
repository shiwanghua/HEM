...
HEM4::HEM4() {
	...
	cout << "ExpID = " << expID << ". HEM4DS: bit exponent = " << be << ", bucketStep = " << buckStep << ", numBucket = " << numBucket << endl;
...
void HEM4::insert(IntervalSub sub)
{
...
void HEM4::initBits() {
	...
	_for(i, 0, numDimension) {
		...
		if (li == numBits) {
			lowContain[li] = 0;
			cout << "HEM4 li=" << li << endl;
		}
		if (hi == numBits) {
			highContain[hi] = numBucket;
			//cout << "HEM4 hi=" << hi << endl;
		}
...
// 不计算时间组成
 void HEM4::match(const Pub& pub, int& matchSubs)
 {
...
// 		{
// 			++matchSubs;
// 			//cout << "HEM4 matches sub: " << i << endl;
// 		}
	matchSubs = subs - b.count();
 }

 // 计算时间组成
 void HEM4::match_debug(const Pub& pub, int& matchSubs)
 {
	 bitset<subs> b, bLocal;
	 vector<bool> attExist(numDimension, false);
	 int value, att, buck;
...
void HEM4::printRelation(int dimension_i) {
	cout << "\n\nHEM4DSMap\n";
	if (dimension_i == -1)
		_for(i, 0, numDimension) {
