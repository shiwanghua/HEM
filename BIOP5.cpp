...
BIOP5::BIOP5()
{
	...
	cout << "ExpID = " << expID << ". BIOP5DD: bit exponent = " << be << ", bucketStep = " << buckStep << ", numBucket = " << numBucket << endl;
...
void BIOP5::initBits()
{
	...
	_for(i, 0, numDimension)
	{
		...
		_for(j, 0, numBucket)
		{
			...
			else if (fix[1][i][j] - fix[1][i][highBktId] <= fix[1][i][highContain[hi]] - fix[1][i][j])
			{ // Bug: 没有减highBktId
