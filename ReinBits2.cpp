...
void ReinBits2::initBits() {
	...
	_for(i, 0, numDimension)
		...
		bid1 = (numBucket - j - 1) / bitStep - 1, bid2 = bid1 + 1;// 2,3; 1,2; 0,1; -1,0;
...
void ReinBits2::printRelation(int dimension_i) {
	...
	if (dimension_i == -1)
		...
		cout << "\nDimension: " << dimension_i << "    Predicates: " << fix[0][dimension_i][numBucket] << "----------------\n";
		_for(j, 0, numBucket)
			cout << "Low Bucket" << j << ": bitsID=" << bitsID[0][dimension_i][j] << ", endBucket=" << endBucket[0][dimension_i][j] << ", doubleReverse=" << doubleReverse[0][dimension_i][j] << endl;
	}
	cout << "\n\n";
}
