...
| 16  | SIMD实验 on HEMDD (匹配时间和或运算时间下降比例-2个图) | $\psi_e=d=[16, 32, 64, 128, 256, 512], blockSize=256,512                                             | 1355-1360, 1361-1366           |
| 17  | pHEMDD 并行实验（加速比和效率-两图）                   | $\psi_e=d=$ 512, pD = [1, 2, 4, 8, 16, 32, 64]                                                       |                                |
| 18  | pHEMDD-avxOR 组合实验（和HEMDD、pHEMDD 比较）          | $\psi_e=d=$ 512, pD = [1, 2, 4, 8, 16, 32, 64], blockSize=512                                        |                                |
| 19  | pHEMDD-RAS-avxOR 高负载高维度实验（和HEMDD-RAS比较）   | n = $2^{27}$, d=2048, attrSubset=32, $\psi_e$=64, pD = [1, 2, 4, 8, 16, 32, 64], $b_e$ = 1           |                                |
...
| 7    | 5/4 - 5/6   | 实现pRein、pHEM5DD、pHEM5DD-avxOR，做实验。                                                                       |
| 8    |             |                                                                                                                   |
