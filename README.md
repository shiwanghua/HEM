...
| 16   | SIMD实验 on HEMDD (匹配时间和或运算时间下降比例-2个图) | $\psi_e=d=[16, 32, 64, 128, 256, 512]                        |                                |
| 17   | pHEMDD 并行实验（加速比和效率-两图）                   | $\psi_e=d=$ 512, pD = [1, 2, 4, 8, 16, 32, 64]               |                                |
| 18   | pHEMDD-avxOR 组合实验（和HEMDD、pHEMDD 比较）          | $\psi_e=d=$ 512, pD = [1, 2, 4, 8, 16, 32, 64]               |                                |
| 19   | pHEMDD-RAS-avxOR 高负载高维度实验（和HEMDD-RAS比较）   | n = $2^{27}$, d=4096, attrSubset=64, $\psi_e$=64, pD = [1, 2, 4, 8, 16, 32, 64] |                                |
