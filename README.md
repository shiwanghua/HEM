...
* Default Parameters: n = 1000000, d = 20, $\phi_s$ = 10, $\phi_e$ = 20, w = 0.4, uniform distribution.
* Model Parameters: 
  * Rein: b = 1000
  * HEM5DD: b = 1000, be = 4
  * Tama: level = 13
...
| ---- | :------------------------------- | ------------------------------------------------------------ | ---------------------------- |
| 1    | 基准验证实验(标记时间、匹配时间) | --                                                           |                              |
| 2    | 空维度验证实验(标记时间)         | $\phi_e$=10, p = 0.5                                         |                              |
| 3    | 订阅数实验                       | n = 300**k** ~ 9**m**, 补: p = 0.5                           |                              |
| 4    | 订阅大小实验                     | $\phi_s$ = 5 ~ 30, d = $S_e$ = 30, w = 0.7, 补: p = 0.5      |                              |
| 5    | 事件大小实验                     | $S_e$ = 30 ~ 80, d = 80, p = 0.5                             |                              |
| 6    | 订阅宽度实验                     | w = 0.1 ~ 0.9, $\phi_s$ = 5                                  |                              |
| 7    | 维度实验                         | d = 30 ~ 900，p = w = 0.5                                    |                              |
| 8(a) | 事件订阅属性Zipf分布实验         | alpha = 0 ~ 5, d = 50, w = 0.5                               |                              |
| 8(b) | 真实事件订阅属性Zipf分布实验     | alpha = 0 ~ 5, d = 50, Ss = 5                                |                              |
| 9    | 综合性能实验                     | $\phi_s$ = 1 ~ 20, $\phi_e$ = 1 ~ 40, w = 0 ~ 1, d = 100, alpha = 1 |                              |
| 10   | 构建时间、删除时间实验(即实验4)  | $\phi_s$ = 5 ~ 30, d = $\phi_e$ = 30, w = 0.7, 补: p = 0.5   |                              |
| 11   | 内存实验(即实验3)                | n = 300**k** ~ 9**m**                                        |                              |
